...
def rle_encode(data):
    ...
    for i in range(1, len(data)):
        if data[i] == data[i - 1]:
            ...
            count = 1
    encoded.append((data[-1], count))  # Add the last element and its count
    return encoded

def main():
    if len(sys.argv) != 2:
        print("Usage: python program-en-rle-int16.py <input_file>")
        sys.exit(1)
    ...
    with open(output_file, 'wb') as f:
        for value, count in encoded_data:
            f.write(struct.pack('<h', value))  # int16 value
            f.write(struct.pack('<H', count))  # unsigned int16 count
